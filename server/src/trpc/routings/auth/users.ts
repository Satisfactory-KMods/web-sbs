...
import { adminProcedure, authProcedure, handleTRCPErr, router } from '@server/trpc/trpc';
...
export const adminUsers = router({
	listUsers: adminProcedure
		...
		.query(async ({ input }) => {
			...
			try {
				const data = (await MongoUserAccount.find(
					{},
					{ salt: 0, hash: 0, email: 0 },
					{ sort: { cratedAt: -1 }, limit, skip }
				)) as ClientUserAccount[];
