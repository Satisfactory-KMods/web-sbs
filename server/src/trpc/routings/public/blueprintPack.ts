...
export const publicBlueprintPacks = router({
	getBlueprintPack: publicProcedure.input(z.object({ blueprintPackId: z.string() })).query(async ({ input }) => {
		...
		try {
			const docu = await MongoBlueprintPacks.findById(blueprintPackId).populate([
				'blueprints',
	...
	getBlueprintPacks: publicProcedure
		...
		.query(async ({ input }) => {
			...
			try {
				...
				const blueprintPacks: BlueprintPackExtended[] = await MongoBlueprintPacks.find<BlueprintPackExtended>(filter, null, {
					...options,
					limit,
					skip
