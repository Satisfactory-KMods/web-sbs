...
export default function () {
	Router.post(
		ApiUrl(EApiBlueprintUtils.parseblueprint),
		MWAuth,
		upload.array('blueprint', 2),
		async (
			req: ExpressRequest<{
	...
	Router.post(
		ApiUrl(EApiBlueprintUtils.create),
		MWAuth,
		upload.fields([
			{ name: 'sbp', maxCount: 1 },
			{ name: 'sbpcfg', maxCount: 1 },
				...
				if (req.files && !Array.isArray(req.files)) {
					...
					for (const file of req.files.images) {
						if (file.mimetype && file.mimetype.split('/')[0] === 'image' && file.mimetype.split('/')[1]) {
							...
							image
								.resize(512 * 1.5, 288 * 1.5)
								.quality(80)
								.write(path.join(blueprintDir, newName));
							fs.rmSync(file.path);
					...
					if (await MongoBlueprints.exists({ originalName: blueprint.originalName })) {
						...
						return res
							.status(500)
							.json(errorResponse('Blueprint with this name is allready in our Database. Please use a other filename!', res));
					}
	...
	Router.post(
		ApiUrl(EApiBlueprintUtils.edit),
		upload.fields([
			{ name: 'sbp', maxCount: 1 },
			{ name: 'sbpcfg', maxCount: 1 },
			{ name: 'images', maxCount: 5 }
			...
			try {
				...
				if (req.files && !Array.isArray(req.files)) {
					...
					if (req.files.sbp && req.files.sbpcfg) {
						...
						fs.existsSync(path.join(blueprintDir, `${id}.sbp`)) &&
