...
export class JobTask {
	...
	constructor(Interval: number, JobName: TTasksRunner, Task: (CallCount: number) => Promise<void>) {
	...
	public UpdateTickTime(NewTime: number) {
		clearInterval(this.Task as any);
	...
	async Init() {
		for (const File of fs.readdirSync(path.join(__BaseDir, '/Tasks/Jobs'))) {
			...
			if (Stats.isFile() && File.endsWith('.Task.ts')) {
				const JobClass: JobTask = (await import(path.join(__BaseDir, '/Tasks/Jobs', File))).default as JobTask;
