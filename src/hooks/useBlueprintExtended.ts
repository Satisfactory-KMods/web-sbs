...
export function useBlueprintExtended(InitValue: BlueprintDataExtended) {
	...
	const allowedToLike = useMemo<boolean>(
		() => loggedIn && !_.isEqual(user.Get._id, blueprint.owner._id),
		[blueprint.owner, loggedIn, user.Get._id]
	);

	const allowedToEdit = useMemo<boolean>(
		() => loggedIn && (_.isEqual(user.Get._id, blueprint.owner._id) || user.HasPermission(ERoles.admin)),
		[blueprint.owner, loggedIn, user]
	);
	...
	const remove = async () => {
		const result = await tRPCAuth.blueprintPacks.remove.mutate({ blueprintPackId: blueprint._id }).then(successSwal).catch(tRPCHandleError);
		return !!result;
	};

	const resolvedMods = useMemo<Mod[]>(
		() => blueprint.mods.map((e) => mods.find((m) => m._id === e)!).filter((e) => !!e) || [],
		[blueprint.mods, mods]
	);
