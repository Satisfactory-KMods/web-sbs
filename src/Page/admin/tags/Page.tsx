...
const Component: FunctionComponent = () => {
	...
	const createTag = async () => {
		setIsFetching(true);
		await tRPCAuth.adminTags.create.mutate({ DisplayName }).then(successSwal).catch(tRPCHandleError);
		await doFetch();
		setIsFetching(false);
	};
	...
	return (
		<div className='flex flex-col'>
			<div className='w-full rounded-lg border border-gray-200 bg-white shadow dark:border-gray-700 dark:bg-gray-800'>
				<div className='rounded-t-lg border-b border-gray-600 bg-gray-700 p-3 text-2xl font-semibold text-neutral-300'>Admin: Tags</div>
				<div className='flex gap-2 p-3'>
					<SBSInput label='Display Name' value={DisplayName} mainClassName='flex-1' onChange={(e: any) => setDisplayName(e.target.value)} />
					<LoadingButton color='green' isLoading={isFetching} onClick={createTag} Icon={FaPlus} disabled={DisplayName.clearWs().length < 1}>
						Add Tag
					</LoadingButton>
				</div>
			</div>
			...
			<div className='mt-2'>
				<Table striped={true}>
					<Table.Head>
						<Table.HeadCell className='w-0'>TagId</Table.HeadCell>
						<Table.HeadCell>Tag</Table.HeadCell>
						<Table.HeadCell className='w-0'>Actions</Table.HeadCell>
					</Table.Head>
					<Table.Body>
						{data.map((tag) => (
							<TagRow key={tag._id} data={tag} doFetch={doFetch} />
						))}
					</Table.Body>
				</Table>
			</div>
