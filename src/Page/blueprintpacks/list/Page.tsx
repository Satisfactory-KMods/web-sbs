	...
	return (
		<>
			<BlueprintFilter filterSchema={[filter, setFilter]} isFetching={isFetching} doFetch={doFetch}>
				<span className='flex-1'>Blueprint Pack Filter ({totalPacks})</span>
				{loggedIn && (
					<Button size='xs' color='green' onClick={() => nav('/blueprintpacks/create')}>
						<FaPlus className='me-2' /> Add a new blueprint pack
					</Button>
				)}
			</BlueprintFilter>
			...
			<div className='mt-3 grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3'>
				{packs.map((pack) => (
					<BlueprintPackCard key={pack._id} Data={pack} onToggled={doFetch} />
				))}
			</div>
