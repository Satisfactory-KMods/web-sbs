...
const ShowBlueprint : FunctionComponent = () => {
	...
	return <Card className={ "mb-3" }>
		...
		<Card.Body className={ "p-0 border-t" }>
			<Table striped border={ 1 } className={ "my-0 table-bordered border-b-0 border-l-0 border-r-0" }>
				<tbody>
				<tr>
				...
				<tr>
					...
					<td>{ Mods.map( R => (
						<Link key={ R._id } to={ `https://ficsit.app/mod/${ R.mod_reference }` } target={ "_blank" }
							...
							<img onError={ E => E.currentTarget.src = "/images/default/unknown.png" }
							     ...
							     className={ "w-10 rounded-l-md" }/><span className={ "px-2 pe-3" }>{ R.name }</span>
						</Link>
					) ) }</td>
				</tr>
				<tr>
		...
		<Card.Footer className={ "p-0" }>
			<ButtonGroup className={ "h-100 w-100" }>
				{ AllowToEdit &&
					<Link to={ `/blueprint/edit/${ Blueprint._id }` } className={ "btn rounded-top-0 btn-dark" }>
						<Icon.BsGearFill/>
					</Link>
				}
