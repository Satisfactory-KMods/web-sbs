...
const validateLogin = async (
	...
	loggedInRule = LoginRule.DontCare,
	redirectTo = '/error/401',
	role?: ERoles
): Promise<LoaderDataBase | Response> => {
...
const validateBlueprint = async (
	...
	redirectTo = '/',
	loggedInRule = LoginRule.DontCare,
	ownerRedirectTo = '/error/401',
	role = ERoles.member
): Promise<LoaderBlueprintBase | Response> => {
...
const validateBlueprintPack = async (
	...
	ownerRedirectTo = '/error/401',
	role = ERoles.member
): Promise<BlueprintPackDefaultLoader | Response> => {
	const loaderBase = await validateLogin({ params, request }, loggedInRule, '/error/401', role);
	if (loaderBase instanceof Response) {
		return loaderBase;
	}
