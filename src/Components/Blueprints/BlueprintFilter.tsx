...
const BlueprintFilter: FunctionComponent<BlueprintFilterProps> = ({ isFetching, filterSchema, doFetch, children, hideTags }) => {
	...
	return (
		<div className='w-full rounded-lg border border-gray-200 bg-white shadow dark:border-gray-700 dark:bg-gray-800'>
			<div className='flex rounded-t-lg border-b border-gray-600 bg-gray-700 p-3 text-2xl font-semibold text-neutral-300'>{children}</div>
			<div className='grid grid-cols-1 gap-2 p-5 text-neutral-200 md:grid-cols-2 lg:grid-cols-3'>
				<SBSInput label='Blueprint Name' value={BlueprintName} onChange={(e: any) => setBlueprintName(e.target.value)}>
					<HiSearch />
				</SBSInput>
				...
				<SBSSelect label='Filter Modded'>
					<Select
						options={vanillaSelectOptions}
						className='gray-select flex-1'
						value={SelectVanilla}
						isClearable={true}
						classNamePrefix='my-react-select'
						onChange={setSelectVanilla}
					/>
				</SBSSelect>

				{SelectVanilla?.value !== true && (
					<SBSSelect label='Mods'>
						<Select
							options={modSelectOptions}
							className='gray-select flex-1'
							value={SelectMods}
							isDisabled={!!SelectVanilla?.value}
							classNamePrefix='my-react-select'
							isMulti={true}
							onChange={setSelectMods}
						/>
					</SBSSelect>
				)}
				...
				<div className='mt-2 flex gap-2 md:mt-0'>
					<LoadingButton isLoading={isFetching} color='gray' className='flex-1' Icon={HiSearch} onClick={applyFilter}>
						Search
					</LoadingButton>
					<LoadingButton isLoading={isFetching} color='red' className='flex-1' Icon={BiTrash} onClick={ResetSearch}>
						Clear
					</LoadingButton>
				</div>
			</div>
