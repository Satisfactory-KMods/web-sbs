...
const UserRow: FunctionComponent<UserRowProps> = ({ data, doFetch }) => {
	...
	const deleteUser = async () => {
		if (await onConfirm('User wirklich löschen?')) {
			await tRPCAuth.adminUsers.delete.mutate({ id: data._id }).then(successSwal).catch(tRPCHandleError);
			await doFetch();
		}
	};

	const modifyRole = async (newRole: number) => {
		await tRPCAuth.adminUsers.setRole
			.mutate({ id: data._id, role: newRole as ERoles })
			.then(successSwal)
	...
	return (
		<Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
			...
			<Table.Cell className='flex gap-2'>
				<LoadingButton disabled={_.isEqual(user.Get._id, data._id)} color='red' isLoading={false} onClick={deleteUser} Icon={BiTrash}>
					Delete
				</LoadingButton>
			</Table.Cell>
		</Table.Row>
